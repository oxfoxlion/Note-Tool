...
export default function UnlockBoardClient({ token }: UnlockBoardClientProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setBusy(true);
      setError('');
      await unlockSharedBoard(token, password);
      router.replace(`/shared-board/${encodeURIComponent(token)}`);
    } catch (err: unknown) {
